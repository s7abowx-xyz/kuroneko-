...
import { githubExchangeCode } from '../../src/oauth';
...
export default async function githubCallback(req: Request, res: Response) {
  ...
  try {
    const profile = await githubExchangeCode(code);
    if (!profile.email) return res.redirect('/login?error=no_email');

    const user = await prisma.user.upsert({
      where: { email: profile.email },
      update: { name: profile.name, avatar: profile.avatar, provider: 'GITHUB', providerId: profile.id },
      create: {
        name: profile.name,
        email: profile.email,
